import { alan, bot, dbio, uoid, utilitas, web } from 'utilitas';
...
const log = (cnt, opt) => utilitas.log(cnt, import.meta.url, { time: 1, ...opt || {} });
const end = bot.end;
...
const paging = (message, options) => {
    ...
    const submit = () => {
        const content = utilitas.trim(lines(page));
    ...
    run: true, priority: -8910, name: 'speech-to-text', func: async (ctx, next) => {
        ...
        if (ctx._.speech?.stt && audio) {
            ...
            try {
                ...
                if (bot._.supportedMimeTypes.has(alan.wav)) {
                    ctx.collect({
    ...
    run: true, priority: -8860, name: 'vision', func: async (ctx, next) => {
        ...
        for (const m of [
            ...
            if (m.photo?.[m.photo?.length - 1]) {
                ...
                files.push({
                    asPrompt: bot._.supportedMimeTypes.has(alan.jpeg),
...
const init = async (options) => {
    if (options) {
        const_bot = bot.init(options);
        if (callosum.isPrimary) {
            // https://github.com/telegraf/telegraf
            const { Telegraf } = await utilitas.need('telegraf', { raw: true });
            // https://github.com/telegraf/telegraf/issues/1736
            const { useNewReplies } = await utilitas.need('telegraf/future', { raw: true });
            ...
            bot.launch();
            callosum.on(BOT_SEND, data => send(...data || []));
            // Graceful stop
            signals.map(signal => process.once(signal, () => end({ signal })));
        } else {
            bot = {
                telegram: { sendMessage: (...args) => callosum.report(BOT_SEND, args) }
            };
        }
    }
