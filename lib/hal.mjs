...
const extendSessionStorage = storage => storage && {
    get: async (id, options) => {
        const CONTEXT_LIMIT = 5;
        const resp = await storage.get(id);
        const ctxResp = await getContext(
            id, undefined, options?.prompt ? CONTEXT_LIMIT : undefined
        );
        const ragResp = await recall(id, options?.prompt, undefined, undefined, {
            exclude: ctxResp.map(x => x.message_id),
        });
        ctxResp.sort((a, b) => ~~a.message_id - ~~b.message_id);
        ragResp.sort((a, b) => a.score - b.score);
        resp.messages = [...ragResp, ...ctxResp];
        print(id, resp);
        return resp;
    },
    set: async (id, session, options) => {
        delete session.messages;
        const resp = await storage.set(id, session, options);
        return resp;
    },
};
