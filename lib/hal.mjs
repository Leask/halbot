...
const memorize = async (ctx) => {
    if (ctx.cmd?.cmd || ctx._skipMemorize) { return; }
    const received = ctx.update;
    const received_text = ctx.txt || '';
    const id = received.update_id;
    let response = {};
    ctx.done.map(m => m?.text && (response[m.message_id] = m));
...
const recall = async (ctx, keyboard, offset = 0) => {
    let result = [];
    switch (hal._?.database?.provider) {
        case dbio.MYSQL:
            result = await hal._.database?.client?.query?.(
                ...
                [keyboard, table, ctx.botInfo.id, ctx.chatId, offset]
            );
            break;
        case dbio.POSTGRESQL:
            // globalThis.debug = 2;
            const vector = await dbio.encodeVector(await hal._.embedding(keyboard));

            result = await hal._.database?.client?.query?.(
                `SELECT *, (1 - (distilled_vector <=> $1)) as relevance `
                + `FROM ${table} WHERE bot_id = $2 AND chat_id = $3`
                + ` AND (distilled_vector <=> $1) < 0.4`
...
const subconscious = [{
    run: true, priority: -8960, name: 'broca', func: async (ctx, next) => {
        ...
        ctx.embedding = hal._.embedding;
        ctx.recall = async (keyboard, offset = 0) => await recall(ctx, keyboard, offset);
    ...
    run: true, priority: -8830, name: 'history', func: async (ctx, next) => {
        ...
        const result = await recall(keyWords, offset);
